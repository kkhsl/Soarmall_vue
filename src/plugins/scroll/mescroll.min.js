...
(function(a, b) {
  ...
  a.prototype.initDownScroll = function() {
    ...
    c.touchmoveEvent = function(l) {
      ...
      if (f > 0) {
        if (d <= 0) {
          c.preventDefault(l);
          if (c.optDown.use && !c.inTouchend && !c.isDownScrolling && !c.optDown.isLock && (!c.isUpScrolling || (
              c.isUpScrolling && c.optUp.isBoth))) {
  ...
  a.prototype.getDomById = function(c) {
    ...
    if (!b) {
      console.error('the element with id as "' + c + '" can not be found: document.getElementById("' + c +
        '")==null')
    }
    return b
  };
