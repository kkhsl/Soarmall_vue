...
("MeScroll", function() {
	...
	a.prototype.initDownScroll = function() {
		...
		c.touchmoveEvent = function(l) {
			...
			if (f > 0) {
				if (d <= 0) {
					c.preventDefault(l);
					if (c.optDown.use && !c.inTouchend && !c.isDownScrolling && !c.optDown.isLock && (!c.isUpScrolling || (c.isUpScrolling &&
							c.optUp.isBoth))) {
				...
				if (f < 0) {
					...
					if (c.optUp.use && !c.optUp.isLock && c.optUp.hasNext && !c.isUpScrolling && (!c.isDownScrolling || (c.isDownScrolling &&
							c.optDown.isBoth)) && (j + c.optUp.offset >= q || i <= 0)) {
						c.triggerUpScroll()
					}
				}
			}
	...
	a.prototype.showEmpty = function() {
		...
		if (g) {
			...
			if (d.logo) {
				f += '<p class="empty-logo">' +'<img class="empty-logo-img" src="' + d.logo + '"/>' + "</p>"
			}
			if (d.btntext) {
				f += '<p class="empty-btn">' + '<img class="empty-logo-img" src="' + d.go + '"/>' + "</p>"
			}
			...
			if (d.btnClick) {
				var e = c.emptyDom.getElementsByClassName("empty-logo")[0];
				if (d.supportTap) {
