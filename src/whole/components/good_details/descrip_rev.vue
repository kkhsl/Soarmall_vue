<template>
  <div class="commodity_pro_rev">
    <div class="pro_rev">
      <div @click="product_()">
        <p :class="coms===1?'hear1':'hear'">{{$t("common.DESCRIPTION")}}</p>
        <p class="dixian" v-if="coms===1"></p>
      </div>
      <div @click="review_()">
        <p :class="coms===2?'hear1':'hear'">{{$t("common.REVIEW")}}</p>
        <p class="dixian" v-if="coms===2"></p>
      </div>
    </div>
    <!-- 商品详情 -->
    <div class="commodity_pro" v-show="coms===1">
      <div class="detailText" :class="{'hideMore': hasMoreDetails}">
        <div class="textWrap" ref="textWrap" v-html="productDetailText" :class="[{'ltr':goods_details_text.substr(0,1) !== '^'},{'showMore':!moreDetailsCheck}]"></div>
        <div class="moreDetails" v-show="hasMoreDetails"><span @click="moreDetailFn">{{moreDetailsCheck?'Unfold' : 'Put away'}}<img :class="{'pullUp':!moreDetailsCheck}" src="@/assets/img/other/xiangzuo.png"/></span></div>
      </div>
      <div class="detailImg" v-html="goods_details_img"></div>
    </div>
    <!-- 好评率 -->
    <div class="commodity_rev" v-show="coms===2">
      <div class="no_data" v-if="goodsdeta.eva_all_count === 0">{{$t("common.No data")}}</div>
      <div class="rev_her" v-if="goodsdeta.eva_all_count !== 0">
        <div class="rev_her_t" @click="rev_p(goodsdeta.goodsid)">
          <div class="rev_her_t_l">
            <span>{{goodsdeta.eva_all_count}}</span>
            <span>{{$t("common.Reviews")}}</span>
          </div>
          <div class="rev_her_t_r">
            <p class="points">Favorable rate:</p>
            <p class="well_eva">{{goodsdeta.well_evaluate*100}}%</p>
            <p class="datb rightIconRow"></p>
          </div>
        </div>
      </div>
      <div class="pj_conter">
        <div class="pj_xh" v-for="(item,index) in items" :key="index">
          <div class="pj_xx">
            <div class="uses">
              <p class="imga">
      ...
      <div class="pj_more" @click="rev_p(goodsdeta.goodsid)" v-if="goodsdeta.eva_all_count">
        {{$t("common.View More")}}(
        <span>{{goodsdeta.eva_all_count}}</span>
        {{$t("common.Reviews")}})
      </div>
...
<script>
export default {
  props: ["goodsdeta", "goods_details_img", "goods_details_text", "items"],
  data() {
    return {
      coms: 1,
  ...
  computed: {
    productDetailText() {
      return this.goods_details_text.substr(0, 1) === "^"
        ? this.goods_details_text.substr(1)
        : this.goods_details_text;
    }
  },
  activated(){
